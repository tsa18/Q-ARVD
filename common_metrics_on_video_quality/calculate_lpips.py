...
# ===================== 核心优化：线程本地存储（TLS）=====================
# 创建线程本地对象，每个线程独立拥有一个loss_fn，只初始化一次
...
def get_lpips_model(device):
    """线程安全的模型获取函数：每个线程只初始化1次模型"""
    # 检查当前线程是否已有模型
    if not hasattr(thread_local, 'loss_fn'):
        # 首次调用：初始化+迁移GPU，仅执行1次/线程
        thread_local.loss_fn = lpips.LPIPS(net='alex', spatial=spatial).to(device)
    return thread_local.loss_fn
...
def calculate_lpips(videos1, videos2, device, only_final=False):
    ...
    # ===================== 替换为线程安全的模型 =====================
    loss_fn = get_lpips_model(device)  # 极速获取，无初始化开销
    # ================================================================
